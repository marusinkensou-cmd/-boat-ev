...
def seed_today(con,now,cache_dir='cache/live',selected_jcds=None):
    ...
    return ok,err
def refresh_for_iphone(con,now=None,max_races=2,horizon_min=90,cache_dir='cache/live',progress=None,selected_jcds=None):
    ...
    for idx,(mins,rid,jcd,rno) in enumerate(selected,1):
        ...
        # Only do exhibition fetch when the race is close enough to plausibly have exhibition data.
        if mins<=40:
            try:
        ...
        # Odds are useful for EV; official page naturally returns all 120 in one fetch.
        try:
        ...
        # A trace is created only because the user explicitly refreshed this race; untouched races are not accumulated.
        try:
        ...
        updates.append(item); report('race_done',race_index=idx,races_total=len(selected),jcd=jcd,race_no=rno)
    board=build_board(con,now); report('done',races_updated=len(updates),venues_found=len(seeded))
    return {'generated_at':now.isoformat(),'settlement':{'skipped_for_speed':True},'actual_stats':cumulative_actual_stats(con),'stats_today':stats(con,today),'stats_all':stats(con),'venues_found':len(seeded),'selected_jcds':list(allowed),'venue_errors':venue_errors,'races_updated':len(updates),'refresh_scope':'selected_venue_next_two','updates':updates,'board':board}
