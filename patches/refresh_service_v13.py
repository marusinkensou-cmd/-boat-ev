...
# Compatibility for the legacy validation module only. The iPhone Update path
# below deliberately does NOT call this helper.
...
def _evaluate_one(con,now,today,mins,rid,jcd,rno,cache_dir,progress=None):
    ...
    if ec<6:
        item['ev']={'status':'predata_not_ready','provisional_model':True,'bets':[],'rows':[]};item['quality']=status(con,rid);item['predata_not_ready']=True;return item
...
def refresh_for_iphone(con,now=None,max_races=1,horizon_min=90,cache_dir='cache/live',progress=None,selected_jcds=None):
    ...
    rows=con.execute('SELECT race_id,jcd,race_no,deadline FROM races WHERE race_date=? AND deadline IS NOT NULL ORDER BY deadline,jcd,race_no',(today,)).fetchall();allowed={str(j).zfill(2) for j in selected_jcds or []};cand=[]
